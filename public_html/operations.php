...
// For removing Clients
  ...
  $sql = "delete from OwnershipHx where clientID = $ID";
  $result = $conn->query($sql);
  if (!$result) {
      trigger_error('ERROR deleting from Clients', E_USER_ERROR);
  }

  $sql = "delete from Art where ownerID = $ID";
  $result = $conn->query($sql);
  if (!$result) {
      trigger_error('ERROR deleting from Clients', E_USER_ERROR);
  }
...
//adding art
  ...
  && $_POST['item3']['restoreKey'] !== '') 
  {
    ...
    $sql = "INSERT INTO OwnershipHx (hashArtID, clientID)
...
//changing ownership of art
    ...
    $sql = "UPDATE OwnershipHx
              SET clientID = '$clientID'
              WHERE hashArtID = '$hashArtID'";
    $result = $conn->query($sql); 
    if (!$result) {
      trigger_error('could not insert into OwnershipHx', E_USER_ERROR);
      return;
    }
...
// list of art owned by client
    ...
    $sql = "select a.artName from Art a where a.ownerID = '" . $clientID . "'";
